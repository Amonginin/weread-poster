...
on:
  schedule:
    # 每天北京时间凌晨 05:00 和 17:00 (对应 UTC 21:00 和 09:00) 自动运行
    - cron: '0 9,21 * * *'
  workflow_dispatch:
```
* **知识点解析**：
...
| :--- | :--- | :--- | :--- |
| **世界标准时换算** | 中国时间减 8 小时即为 UTC 触发时间 | [.github/workflows/weread_poster.yml](file:///d:/coding/personalProj/weread-poster/.github/workflows/weread_poster.yml#L5) | `cron: '0 9,21 * * *'` 对应北京 17 点和 5 点 |
...
### 题目 1：在配置定时 Cron `'0 9,21 * * *'` 时，为什么不推荐将其写为零整点，如 `'0 0 * * *'`？
